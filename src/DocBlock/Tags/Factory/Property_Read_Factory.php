...
use Php_Documentor\Reflection\Doc_Block\Tags\Property_Read;
 ...
 */
final class Property_Read_Factory implements Php_Stan_Factory
{
    ...
    public function create(Php_Doc_Tag_Node $node, Context $context): Tag
    {
        ...
        return new Property_Read(trim($tag_value->property_name, '$'), $this->type_resolver->create_type($tag_value->type, $context), $this->description_factory->create($description, $context));
    }
    public function supports(Php_Doc_Tag_Node $node, Context $context): bool
    {
        return $node->value instanceof Property_Tag_Value_Node && $node->name === '@property-read';
    }
}