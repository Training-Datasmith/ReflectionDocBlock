...
use Php_Documentor\Reflection\Doc_Block\Tags\Mixin;
 ...
 */
final class Mixin_Factory implements Php_Stan_Factory
{
    ...
    public function create(Php_Doc_Tag_Node $node, Context $context): Tag
    {
        ...
        Assert::is_instance_of($tag_value, Mixin_Tag_Value_Node::class);
        $description = $tag_value->get_attribute('description');
        if (is_string($description) === false) {
            $description = $tag_value->description;
        }
        return new Mixin($this->type_resolver->create_type($tag_value->type, $context), $this->description_factory->create($description, $context));
    }
    public function supports(Php_Doc_Tag_Node $node, Context $context): bool
    {
        return $node->value instanceof Mixin_Tag_Value_Node;
    }
}