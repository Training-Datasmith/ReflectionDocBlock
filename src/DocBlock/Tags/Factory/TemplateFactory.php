...
use Php_Documentor\Reflection\Doc_Block\Tags\Template;
 ...
 */
final class Template_Factory implements Php_Stan_Factory
{
    ...
    public function create(Php_Doc_Tag_Node $node, Context $context): Tag
    {
        ...
        return new Template($name, $this->type_resolver->create_type($tag_value->bound, $context), $this->type_resolver->create_type($tag_value->default, $context), $this->description_factory->create($description, $context));
    }
    public function supports(Php_Doc_Tag_Node $node, Context $context): bool
    {
        return $node->value instanceof Template_Tag_Value_Node && $node->name === '@template';
    }
}