...
use Php_Documentor\Reflection\Doc_Block\Tags\Implements_;
 ...
 */
final class Implements_Factory implements Php_Stan_Factory
{
    ...
    {
        return $node->value instanceof Implements_Tag_Value_Node && $node->name === '@implements';
    }
    public function create(Php_Doc_Tag_Node $node, Context $context): Tag
    {
        $tag_value = $node->value;
        Assert::is_instance_of($tag_value, Implements_Tag_Value_Node::class);
        $description = $tag_value->get_attribute('description');
        if (is_string($description) === false) {
            $description = $tag_value->description;
        }
        return new Implements_($this->type_resolver->create_type($tag_value->type, $context), $this->description_factory->create($description, $context));
    }
}