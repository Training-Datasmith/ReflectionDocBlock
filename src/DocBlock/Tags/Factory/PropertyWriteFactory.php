...
use Php_Documentor\Reflection\Doc_Block\Tags\Property_Write;
 ...
 */
final class Property_Write_Factory implements Php_Stan_Factory
{
    ...
    public function create(Php_Doc_Tag_Node $node, Context $context): Tag
    {
        ...
        Assert::is_instance_of($tag_value, Property_Tag_Value_Node::class);
        $description = $tag_value->get_attribute('description');
        if (is_string($description) === false) {
            $description = $tag_value->description;
        }
        return new Property_Write(trim($tag_value->property_name, '$'), $this->type_resolver->create_type($tag_value->type, $context), $this->description_factory->create($description, $context));
    }
    public function supports(Php_Doc_Tag_Node $node, Context $context): bool
    {
        return $node->value instanceof Property_Tag_Value_Node && $node->name === '@property-write';
    }
}